...
class PerformColorAug(torch.nn.Module):
    
    def __init__(self, params):
        super(PerformColorAug, self).__init__()
        self.params = params
...
class MonoDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.load_mask_triplet = load_mask_triplet
    ...
    def __getitem__(self, index):
        ...
        folder, frame_index, side, object_id_list = self.index_to_folder_and_frame_idx(index)

        poses = {}
        if 'cityscapes' in type(self).__name__.lower():
            inputs.update(self.get_colors(folder, frame_index, side, object_id_list, do_flip))
        elif 'waymo' in type(self).__name__.lower():
            inputs.update(self.get_colors(folder, frame_index, side, do_flip))
        elif 'kitti' in type(self).__name__.lower():
        ...
        if do_color_aug:
            color_aug = PerformColorAug(transforms.ColorJitter.get_params(
                self.brightness, self.contrast, self.saturation, self.hue))
        else:
        ...
        inputs['frame_id'] = self.filenames[index] # f'{folder} {frame_index}'
        return inputs
