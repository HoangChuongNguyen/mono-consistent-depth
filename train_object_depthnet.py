...
class ObjectDepthTrainer:
    def __init__(self, cfg, trainer_pixel_wise=None):
        ...
        self.trainer_pixel_wise = trainer_pixel_wise
    ...
    def retrieve_corresponding_mask(self, split):
        ...
        with torch.no_grad():
            for i, inputs in enumerate(tqdm(object_dataloader)):
                # Process batch
                inputs, outputs = self.trainer_pixel_wise.process_batch(inputs, epoch=10000, get_prediction_only=True)
    ...
    def get_valid_train_mask_file(self, split):
        ...
        if split == 'train': 
            file_name_list = self.trainer_pixel_wise.train_file
        elif split == 'val': 
            file_name_list = self.trainer_pixel_wise.val_file
        else: 
    ...
    def training_setup(self):
        ...
        if self.trainer_pixel_wise: 
            del self.trainer_pixel_wise
    ...
    def predict_pose(self, inputs, predict_pos_pose=True):
        ...
        # We do not predict pose for manydepth while it is in evaluation mode
        if predict_pos_pose:
